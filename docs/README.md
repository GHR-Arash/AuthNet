...
The first MVP slice targets .NET 10 and plugs into ASP.NET applications as middleware. It provides ASP.NET Core Identity integration, Razor Pages account UI, admin user-management UI, account invitations, cookie authentication, PostgreSQL-backed EF Core persistence, role infrastructure, generic OpenID Connect login, authenticator-app MFA, and email-based account flows.

API/JWT, refresh tokens, SPA flows, custom persistence stores, and provider-specific helpers are deferred.
